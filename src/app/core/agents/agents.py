...
def planning_node(state: QAState) -> QAState:
    ...
    messages = result.get("messages", [])
    output = _extract_last_ai_content(messages)

    plan = ""
    sub_questions: list[str] = []

    if "Sub-questions:" in output:
        plan_part, sub_part = output.split("Sub-questions:", 1)
        plan = plan_part.replace("Plan:", "").strip()

        for line in sub_part.splitlines():
            line = line.strip()
            if line.startswith("-"):
    ...
    # Visible logging (acceptance criteria)
    print("\n[QUERY PLAN]")
    print(plan)
    print("[SUB-QUESTIONS]")
